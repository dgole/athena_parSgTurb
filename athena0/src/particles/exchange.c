...
extern void RemapFlux_2ndorder(const Real *U,const Real eps,const int ji,const int jo, Real *F);
#endif
  ...
  for(k=kb; k<=kt; k++){
   for(i=0; i<NExc+NOfst; i++){
	...
	UBuf[j] = TempZns[k][i][j].U[n];
      ...
      RemapFlux_2ndorder(UBuf,epso,js,je+1,Flx);
      for(j=js; j<=je; j++){
  ...
  for(k=kb; k<=kt; k++){
   for(i=0; i<NExc+NOfst; i++){
	...
	UBuf[j] = TempZns[k][i][j].U[n];
      ...
      RemapFlux_2ndorder(UBuf,epsi,js,je+1,Flx);
      for(j=js; j<=je; j++){
        TempZns[k][i][j].U[n] -= (Flx[j+1] - Flx[j]);
      }
  }}}
