...
/* Step 1: Calculate velocity update */
    switch(grproperty[curG->property].integrator)
    {
      case 1: /* 2nd order explicit integrator */
        int_par_exp(pD, pG, curG, cell1, &dv1, &dv2, &dv3, &ts, p);
        break;

      case 2: /* 2nd order semi-implicit integrator */
        int_par_semimp(pD, pG, curG, cell1, &dv1, &dv2, &dv3, &ts, p);
        break;

      case 3: /* 2nd order fully implicit integrator */
        int_par_fulimp(pD, pG, curG, cell1, &dv1, &dv2, &dv3, &ts, p);
        break;
...
#ifdef FEEDBACK
   feedback_corrector(pG, curG, curP, cell1, dv1, dv2, dv3, ts, p);
#endif /* FEEDBACK */
...
//    if (curP->x1 < 0.5 && curP->x1 > -0.5 && curP->x2 < 0.5 && curP->x2 > -0.5 && curP->x3 < 0.5 && curP->x3 > -0.5) {
//      if ((curP->x1 < 0 && curP->v1 < 0) || (curP->x1 > 0 && curP->v1 > 0)) { 
//        printf("v1 = %g, v2 = %g, v3 = %g, x1=%g,x2=%g,x3=%g\n",curP->v1,curP->v2,curP->v3,curP->x1,curP->x2,curP->x3);
//      }
//    }
 ...
 *                            Real *dv1, Real *dv2, Real *dv3, Real *ts, int p)
 ...
 */
void int_par_fulimp(DomainS *pD, GridS *pG, GrainS *curG, Real3Vect cell1, 
                              Real *dv1, Real *dv2, Real *dv3, Real *ts, int p)
{
  ...
  Real3Vect fsg;        /* Force due to self-gravity */
#endif
...
/* Step 2.5: Invoke self-gravity of particles */
#ifdef PARTICLE_SELF_GRAVITY
  fsg = Get_Force_SG(pG, curG->x1, curG->x2, curG->x3, cell1);
#endif
...
/* Step 3.5: Invoke self-gravity of particles */
#ifdef PARTICLE_SELF_GRAVITY
  fsg = Get_Force_SG(pG, x1n, x2n, x3n, cell1);
#endif
 ...
 *   dv1,dv2,dv3: velocity update
 */
void int_par_semimp(DomainS *pD, GridS *pG, GrainS *curG, Real3Vect cell1, 
                              Real *dv1, Real *dv2, Real *dv3, Real *ts, int p)
{
 ...
 *   dv1,dv2,dv3: velocity update
 */
void int_par_exp(DomainS *pD, GridS *pG, GrainS *curG, Real3Vect cell1,
                           Real *dv1, Real *dv2, Real *dv3, Real *ts, int p)
{
  Real3Vect fd, fr, ft;	/* drag force and other forces, total force */
...
void feedback_predictor(DomainS *pD)
{
  ...
  Real stiff_factor;        /* Prefactor for stiffness to increase stability while not changing drag physics */
...
// MODIFIED BY JB SIMON TO INCREASE STABILITY
      stiffness = m*pG->dt/tstop;

      /* Here is a prefactor to remove the dt/tstop and increase stability */
      stiff_factor = MAX(stiff_factor,tstop/pG->dt);
  ...
  for (k=klp; k<=kup; k++)
    for (j=jlp; j<=jup; j++)
      for (i=ilp; i<=iup; i++)
      {
        ...
        stiffness = 1.0/MAX(1.0, pG->Coup[k][j][i].FBstiff*stiff_factor);
 ...
 *                    Real3Vect cell1, Real dv1, Real dv2, Real dv3, Real ts, int p)
...
#ifdef PARTICLE_SELF_GRAVITY
  fsg = Get_Force_SG(pG, x1, x2, x3, cell1);
  fb.x1 += fsg.x1;
  fb.x2 += fsg.x2;
  fb.x3 += fsg.x3;
#endif
