...
Real x1min,x1max,x2min,x2max,x3min,x3max,Lx,Ly,Lz,Lg; // RL: act like static
long Npar;
/* output variables */
...
void problem(DomainS *pDomain)
{
    ...
    // RL: Even with NO_MPI_PARALLEL, myID_Comm_world has a value, 0. So I put it outside.
    ...
    if (tsmode == 3) {/* fixed stopping time */
        ...
        for (i=0; i<npartypes; i++) {
            ...
            printf("tstop0[%d]=%f, grproperty[%d].rad=%f\n", i, tstop0[i], i, grproperty[i].rad); // RL: add a new line for output
        ...
        for (i=0; i<npartypes; i++)
            grproperty[i].rad = amin*exp(i*log(amax/amin)/MAX(npartypes-1,1.0)); printf("grproperty[%d].rad=%f\n", i, grproperty[i].rad); // RL: add a printf()
    ...
    for (i=0; i<npartypes; i++)
    {
        ...
        printf("ScaleHg=%f, Lz=%f, ep[%d]=%f, Npar=%ld, m=%f\n", ScaleHg, Lz, i, ep[i], Npar, grproperty[i].m); // RL: add a new line for output
    }
#else
    mratio = 0.0;
    for (i=0; i<npartypes; i++)
...
void Userwork_in_loop(MeshS *pM)
{
    ...
    // RL: output gas mass loss
    ath_pout(0, "mratio = %21.16e\n", mratio);
