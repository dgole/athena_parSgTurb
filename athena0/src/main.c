 ...
 * a prolongation step to set ghost zones at internal fine/coarse boundaries  */
  ...
  particle_self_gravity_init(&Mesh); 
  bvals_particle_self_gravity_init(&Mesh);
#ifdef SHEARING_BOX
  bvals_particle_self_gravity_shear_init(&Mesh);
#endif
#endif 
...
/*--- Step 9c. ---------------------------------------------------------------*/
/* Compute gravitational potential of particle density */
    ...
    for (nl=0; nl<(Mesh.NLevels); nl++){
      for (nd=0; nd<(Mesh.DomainsPerLevel[nl]); nd++){
        if (Mesh.Domain[nl][nd].Grid != NULL){
          ...
          selfg_particle_fft_3d(&(Mesh.Domain[nl][nd]));
#endif
#ifdef PARTICLE_SELF_GRAVITY_USING_FFT_OBC
          selfg_particle_fft_obc_3d(&(Mesh.Domain[nl][nd]));
#endif
#ifdef PARTICLE_SELF_GRAVITY_USING_FFT_DISK
          selfg_particle_fft_disk_3d(&(Mesh.Domain[nl][nd]));
#endif
          bvals_particle_self_gravity(&(Mesh.Domain[nl][nd]));
          Calculate_Force_SG(&(Mesh.Domain[nl][nd]));
        }
      }
    }
#endif
...
#endif
#ifdef PARTICLE_SELF_GRAVITY
  particle_self_gravity_destruct();
#ifdef SHEARING_BOX
  bvals_particle_self_gravity_shear_destruct();
#endif
#endif
#if defined(SHEARING_BOX) || (defined(FARGO) && defined(CYLINDRICAL))
