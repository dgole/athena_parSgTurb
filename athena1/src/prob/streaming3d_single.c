...
/* Read initial conditions */
 ...
 turned on!\n");
  ...
  {
    if (Lx != Lz)
      ath_error("[streaming3d]: Linear test requires Lx1=Lx2!\n");
...
/* Now set initial conditions for the gas */
  ...
  for (i=pGrid->is; i<=pGrid->ie; i++) {
    ...
    if ((ipert == 1) || (ipert == 2)) {
      rhog = rho0 * (1.0+reduct*pert_even(Rerho,Imrho,x1,x3,t));
      u1 = etavk * reduct * pert_even(Reux,Imux,x1,x3,t);
      u3 = etavk * reduct * pert_odd (Reuz,Imuz,x1,x3,t);
      u2 = etavk * reduct * pert_even(Reuy,Imuy,x1,x3,t);
    } else {
...
/* enroll gravitational potential function, shearing sheet BC functions */
  ...
  if (myID_Comm_world == 0) {
    /* flush output file */
    sprintf(name, "%s_%d_%d.dat", "Streaming3d", Nx,ipert);
